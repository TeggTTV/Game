...
class ItemAddedToInventoryHint extends Hint {
    ...
    draw() {
        ...
        ctx.fillRect(background.x, background.y, background.width + margin * 2, background.height + margin * 2);
        ctx.fillStyle = "black";
        for (let i = 0; i < this.descriptions.length; i++) {
            ctx.fillText(this.descriptions[i].text, background.x + margin, background.y + margin + i * first.height + first.height);
        }
        ctx.fillStyle = "white";
        for (let i = 0; i < this.descriptions.length; i++) {
            ctx.fillText(this.descriptions[i].text, background.x + margin, background.y + margin + i * first.height + first.height);
...
//# sourceMappingURL=ItemAddedToInventoryHint.js.map